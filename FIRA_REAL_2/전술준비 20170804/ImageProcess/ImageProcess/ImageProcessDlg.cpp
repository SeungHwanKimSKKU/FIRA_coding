...
BEGIN_MESSAGE_MAP(CImageProcessDlg, CDialogEx)
	...
	ON_STN_CLICKED(IDC_MAIN, &CImageProcessDlg::OnStnClickedMain)
END_MESSAGE_MAP()
...
int CImageProcessDlg::ThreadFunction2(void)
{
	...
	void CImageProcessDlg::ImageProcess(CvSize img_size)
	{
		...
		//템플릿 매칭//
		///*
		//IplImage * image = &IplImage(); // input : 카메라에 찍힌 영상 // 카메라 영상을 잘모르겠당.... Mat에서 IplImage 변환한건 배웠어도.. ㅠ
		//IplImage * out = cvCreateImage(cvSize(image->width,image->height),image->depth,1); //입력영상 흑백 준비

		//IplImage * src = cvLoadImage("C:\\opencv\\TennisBall.jpg");
		////IplImage * pic = cvCreateImage(cvSize(src->width, src->height),src->depth,1); //테니스 공사진 흑백 준비
		//if ((src = cvLoadImage("C:\\opencv\\TennisBall.jpg")) == 0) 
		//{
		//	printf("ERRORS ON READING INPUT IMAGE","C:\\opencv\\TennisBall.jpg");
		//}

		//cvCvtColor(image, out, CV_RGB2GRAY); //카메라에 찍힌 영상 흑백으로 전환
		////cvCvtColor(src,pic,CV_RGB2GRAY); //테니스 공사진 흑백으로 전환
		//
		//int iwidth = out->width - pic->width +1;
		//int iheight = out->height - pic->height +1;

		//IplImage * out2 = cvCreateImage(cvSize(iwidth, iheight), 32, 1);

		//cvMatchTemplate(out,pic,out2,CV_TM_CCORR_NORMED);
		//CvPoint left_top;
		//double min, max;
		//cvMinMaxLoc(out2,&min,&max,NULL,&left_top);
		//cvRectangle(out, left_top, cvPoint(left_top.x + pic->width, left_top.y+pic->height ),CV_RGB(0,0,255));
		//	cvCircle(out,); //원모양.. 그런데 중심좌표..ㅠ
		//cvNamedWindow("VIDEO",CV_WINDOW_AUTOSIZE); //카메라에 찍힌 영상이 창에뜰거임
		//cvNamedWindow("Temp",CV_WINDOW_AUTOSIZE);
		//cvShowImage("VIDEO",image);
		//cvShowImage("Temp",out);
		////*/		
		...
		if (Flag_Run == 1)
		{
			...
			for (k = 0; k < 3; k++)
			{
				if (count_Blue[k]>400) //세어진 각 레이블의 파란색 개수의 크기로 로봇아이디 구별
				{
					...
					if (count_front_Green[0] != 0 || count_front_Pink != 0)
					{
						if (count_front_Green[0] > count_front_Pink)
						{
							...
							*Robot_ID[k] = 1;
							...
							P_Robot1_a = angle_find(center_front_Green_x[0], center_front_Green_y[0], Robot_Info[0][0], Robot_Info[0][1]);
						...
						else
						{
							...
							*Robot_ID[k] = 2;
							...
							P_Robot2_a = angle_find(center_front_Pink_x, center_front_Pink_y, Robot_Info[1][0], Robot_Info[1][1]);
				...
				else if (20 < count_Blue[k] && count_Blue[k] < 300)
				{
					*Robot_ID[k] = 3;
			...
			/////////////////////////
			////////////////////////
			/*P_BY_x1 = center_Blue_x[0];
			P_BY_x2 = center_Blue_x[1];
			P_BY_x3 = center_Blue_x[2];
			P_BY_y1 = center_Blue_y[0];
			P_BY_y2 = center_Blue_y[1];
			P_BY_y3 = center_Blue_y[2];*/

			//int total_Red_x = 0, total_Red_y = 0;// x, y좌표의 합, ID별 숫자 들어감
			//int count_Red = 0;// ID별 픽셀의 개수
			//int center_Red_x = 0, center_Red_y = 0;// ID별 빨간색의 중심좌표
			//int total_Green_x = 0, total_Green_y = 0;// x, y좌표의 합, ID별 숫자 들어감
			//int count_Green = 0;// ID별 픽셀의 개수
			//int center_Green_x = 0, center_Green_y = 0;// ID별 초록색의 중심좌표
			//int total_Pink_x = 0, total_Pink_y = 0;// x, y좌표의 합, ID별 숫자 들어감
			//int count_Pink = 0;// ID별 픽셀의 개수
			//int center_Pink_x = 0, center_Pink_y = 0;// ID별 핑크색의 중심좌표
			...
			//				total_Red_y += i;
			//				total_Red_x += j;
			//				center_Red_x = (int)(total_Red_x / count_Red);
			//				center_Red_y = (int)(total_Red_y / count_Red);
			//				//id == 1 
			//			}
			//			//초록색
			//			else if (Cr_Green_Th_L < data_Cr[i*width + j] && data_Cr[i*width + j] < Cr_Green_Th_U && Cb_Green_Th_L < data_Cb[i*width + j] && data_Cb[i*width + j] < Cb_Green_Th_U)
			//			{
			//				count_Green += 1;
			//				total_Green_y += i;
			//				total_Green_x += j;
			//				center_Green_x = (int)(total_Green_x / count_Green);
			//				center_Green_y = (int)(total_Green_y / count_Green);
			//				//id == 2
			//			}
			//			//핑크색
			//			else if (Cr_Pink_Th_L < data_Cr[i*width + j] && data_Cr[i*width + j] < Cr_Pink_Th_U && Cb_Pink_Th_L < data_Cb[i*width + j] && data_Cb[i*width + j] < Cb_Pink_Th_U)
			//			{
			//				count_Pink += 1;
			//				total_Pink_y += i;
			//				total_Pink_x += j;
			//				center_Pink_x = (int)(total_Pink_x / count_Pink);
			//				center_Pink_y = (int)(total_Pink_y / count_Pink);
			//				//id == 3
			//			}
			//		}
			//	}
			...
			//	if (i == P_BY_y1 + ROI - 1)
			//	{
			//		//printf("첫번째 파란색 center_Blue_x : %d  center_Blue_y : %d\n", center_Blue_x[0], center_Blue_y[0]);
			//		//printf("count_Red : %d, count_Green : %d, count_Pink : %d \n", count_Red, count_Green, count_Pink);
			//		if (count_Red > count_Green && count_Red > count_Pink && red_count == 0)
			//		{
			//			//printf("빨간색 center_Red_x : %d  center_Red_y : %d\n", center_Red_x, center_Red_y);
			//			P_Robot1_ID = 1;
			//			Robot_Info[0][0] = (center_Blue_x[0] + center_Red_x) / 2;
			//			Robot_Info[0][1] = (center_Blue_y[0] + center_Red_y) / 2;
			//			P_Red_x1 = center_Red_x;
			//			P_Red_y1 = center_Red_y;
			//			red_count += 1;
			//		}
			//		else if (count_Green > count_Red && count_Green > count_Pink && green_count == 0)
			//		{
			//			//printf("초록색 center_Green_x : %d  center_Green_y : %d\n", center_Green_x, center_Green_y);
			//			P_Robot1_ID = 2;
			//			Robot_Info[1][0] = (center_Blue_x[0] + center_Green_x) / 2;
			//			Robot_Info[1][1] = (center_Blue_y[0] + center_Green_y) / 2;
			//			P_Red_x1 = center_Green_x;
			//			P_Red_y1 = center_Green_y;
			//			green_count += 1;
			//		}
			//		else if (count_Pink > count_Green && count_Pink > count_Red && pink_count == 0)
			//		{
			//			printf("핑크색 center_Pink_x : %d  center_Pink_y : %d\n", center_Pink_x, center_Pink_y);
			//			P_Robot1_ID = 3;
			//			Robot_Info[2][0] = (center_Blue_x[0] + center_Pink_x) / 2;
			//			Robot_Info[2][1] = (center_Blue_y[0] + center_Pink_y) / 2;
			//			P_Red_x1 = center_Pink_x;
			//			P_Red_y1 = center_Pink_y;
			//			pink_count += 1;
			//		}
			//		count_Red = 0;
			//		count_Green = 0;
			...
			//}
			//// 두번째 파란색의 ID구하기
			//for (i = P_BY_y2 - ROI; i < P_BY_y2 + ROI; i++)			// 파란색 영역의 중심에서 ROI만큼 떨어진 i값의 범위안에서 탐색
			//{
			//	for (j = P_BY_x2 - ROI; j < P_BY_x2 + ROI; j++)		// 파란색 영역의 중심에서 ROI만큼 떨어진 j값의 범위안에서 탐색
			//	{
			//		if (0 < i && i < height && 0 < j &&j < width)	// i나 j가 (-)값이나 이미지 크기를 벗아난 값을 가질 때 나타나는 에러를 피하기 위해 제외
			//		{
			//			//빨간색
			...
			//	if (i == P_BY_y2 + ROI - 1)
			//	{
			//		//printf("두번째 파란색 center_Blue_x : %d  center_Blue_y : %d\n", center_Blue_x[1], center_Blue_y[1]);
			//		//printf("count_Red : %d, count_Green : %d, count_Pink : %d \n", count_Red, count_Green, count_Pink);
			//		if (count_Red > count_Green && count_Red > count_Pink && red_count == 0)
			//		{
			//			//printf("빨간색 center_Red_x : %d  center_Red_y : %d\n", center_Red_x, center_Red_y);
			//			P_Robot2_ID = 1;
			//			Robot_Info[0][0] = (center_Blue_x[1] + center_Red_x) / 2;
			//			Robot_Info[0][1] = (center_Blue_y[1] + center_Red_y) / 2;
			//			P_Red_x2 = center_Red_x;
			//			P_Red_y2 = center_Red_y;
			//			red_count += 1;
			//		}
			//		else if (count_Green > count_Red && count_Green > count_Pink && green_count == 0)
			//		{
			//			//printf("초록색 center_Green_x : %d  center_Green_y : %d\n", center_Green_x, center_Green_y);
			//			P_Robot2_ID = 2;
			//			Robot_Info[1][0] = (center_Blue_x[1] + center_Green_x) / 2;
			//			Robot_Info[1][1] = (center_Blue_y[1] + center_Green_y) / 2;
			//			P_Red_x2 = center_Green_x;
			//			P_Red_y2 = center_Green_y;
			//			green_count += 1;
			//		}
			//		else if (count_Pink > count_Green && count_Pink > count_Red && pink_count == 0)
			//		{
			//			//printf("핑크색 center_Pink_x : %d  center_Pink_y : %d\n", center_Pink_x, center_Pink_y);
			//			P_Robot2_ID = 3;
			//			Robot_Info[2][0] = (center_Blue_x[1] + center_Pink_x) / 2;
			//			Robot_Info[2][1] = (center_Blue_y[1] + center_Pink_y) / 2;
			//			P_Red_x2 = center_Pink_x;
			//			P_Red_y2 = center_Pink_y;
			//			pink_count += 1;
			//		}
			//		count_Red = 0;
			//		count_Green = 0;
			...
			//}
			//// 세번째 파란색의 ID구하기
			//for (i = P_BY_y3 - ROI; i < P_BY_y3 + ROI; i++)			// 파란색 영역의 중심에서 ROI만큼 떨어진 i값의 범위안에서 탐색
			//{
			//	for (j = P_BY_x3 - ROI; j < P_BY_x3 + ROI; j++)		// 파란색 영역의 중심에서 ROI만큼 떨어진 j값의 범위안에서 탐색
			//	{
			//		if (0 < i && i < height && 0 < j &&j < width)	// i나 j가 (-)값이나 이미지 크기를 벗아난 값을 가질 때 나타나는 에러를 피하기 위해 제외
			//		{
			//			//빨간색
			...
			//	if (i == P_BY_y3 + ROI - 1)
			//	{
			//		//printf("세번째 파란색 center_Blue_x : %d  center_Blue_y : %d\n", center_Blue_x[2], center_Blue_y[2]);
			//		//printf("count_Red : %d, count_Green : %d, count_Pink : %d \n", count_Red, count_Green, count_Pink);
			//		if (count_Red > count_Green && count_Red > count_Pink && red_count == 0)
			//		{
			//			//printf("빨간색 center_Red_x : %d  center_Red_y : %d\n", center_Red_x, center_Red_y);
			//			P_Robot3_ID = 1;
			//			Robot_Info[0][0] = (center_Blue_x[2] + center_Red_x) / 2;
			//			Robot_Info[0][1] = (center_Blue_y[2] + center_Red_y) / 2;
			//			P_Red_x3 = center_Red_x;
			//			P_Red_y3 = center_Red_y;
			//			red_count += 1;
			//		}
			//		else if (count_Green > count_Red && count_Green > count_Pink && green_count == 0)
			//		{
			//			//printf("초록색 center_Green_x : %d  center_Green_y : %d\n", center_Green_x, center_Green_y);
			//			P_Robot3_ID = 2;
			//			Robot_Info[1][0] = (center_Blue_x[2] + center_Green_x) / 2;
			//			Robot_Info[1][1] = (center_Blue_y[2] + center_Green_y) / 2;
			//			P_Red_x3 = center_Green_x;
			//			P_Red_y3 = center_Green_y;
			//			green_count += 1;
			//		}
			//		else if (count_Pink > count_Green && count_Pink > count_Red && pink_count == 0)
			//		{
			//		//	printf("핑크색 center_Pink_x : %d  center_Pink_y : %d\n", center_Pink_x, center_Pink_y);
			//			P_Robot3_ID = 3;
			//			Robot_Info[2][0] = (center_Blue_x[2] + center_Pink_x) / 2;
			//			Robot_Info[2][1] = (center_Blue_y[2] + center_Pink_y) / 2;
			//			P_Red_x3 = center_Pink_x;
			//			P_Red_y3 = center_Pink_y;
			//			pink_count += 1;
			//		}
			//		count_Red = 0;
			//		count_Green = 0;
	...
	const unsigned long Motion_table[32] = {
		//0~31
		0x000, 0x000, 0x000, 0x000, 0x000, 0x000002, 0x000, 0x000,//0~7
		0x000, 0x000, 0x000, 0x000, 0x000, 0x000, 0x000, 0x000,//8~15
		0x000, 0x000, 0x000, 0x000080, 0x000, 0x000, 0x000, 0x000,//16~23
	...
	bool CImageProcessDlg::SendData(int index, int commend)
	{
		...
		Send_MotionRemoteControl_Write_NoResponse(index + 1, 0, Motion_table[commend], 100);//////고영 로봇 제어문 패킷으로 바꾼다
		return true;
	...
	void CImageProcessDlg::RobotMove(int num, int Target_x, int Target_y, int pixel_gap_x)
	{
	...
	void CImageProcessDlg::LR_CHECK(int num, int Target_x, int Target_y, int pixel_gap_l_y, int pixel_gap_r_y)
	{
	}
	...
	void XY_Find(int x, int y, int ID)
	{
		double high = 247 * 3.1, center_x = 512, center_y = 289, Robot_high = 3.1 * 40;
	...
	void CImageProcessDlg::OnStnClickedMain()
	{
		// TODO: 여기에 컨트롤 알림 처리기 코드를 추가합니다.
	}
