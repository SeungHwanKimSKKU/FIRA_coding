...
int CImageProcessDlg::ThreadFunction2(void)
{
	...
	while (Flag_Thread_State == 1)
	{
		...
		/*if (abs(Robot_Info[0][0] - P_Ball_x) > 1)
		{
			...
			if (flag_time3 == 1) //time_interval2를 기준 간격으로 명령어 전달하는 부분 - Robot 함수 전달
			{
				...
				if (Keeper_Sign == 0)
				{
					if ((730 > N_Ball_x1 && N_Ball_x1 > 680) || (900 > N_Ball_x2 && N_Ball_x2 > 680) || (900 > N_Ball_x3 && N_Ball_x3 > 680))
					{
						SendData(1, 2);
						Keeper_Sign = 1;
					}
			...
			if (flag_time4 == 1) //time_interval2를 기준 간격으로 명령어 전달하는 부분 - Robot 함수 전달
			{
				...
				//	//
				if (P_Ball_x >= 520 && P_Ball_x <= 800 && P_Ball_y >= 55 && P_Ball_y <= 490)
				{
					if (G_x - 15 < Robot_Info[2][0] && Robot_Info[2][0] <= G_x + 15 && G_y - 15 < Robot_Info[2][1] && Robot_Info[2][1] <= G_y + 15)
					{
						//G_Sign[0] = 1;
						//}
						//if (G_Sign[0] == 1)
					...
					else
					{
						...
						if (Robot_Info[2][0] < P_Ball_x)
						{
							...
							{
								RobotMove(2, P_Ball_x + 50, P_Ball_y + 50);
							}
							else
							{
				...
				else
				{
					...
					//else
					{
						Target_Move(2, 660, 280, 1);
						if (30 > abs(Robot_Info[2][1] - 280) && abs(Robot_Info[2][0] - 420) < 30)
						{
							//time_interval4 = 900;
							Angle_Check(0, 180);
	...
	void CImageProcessDlg::ImageProcess(CvSize img_size)
	{
		...
		if (Flag_Run == 1)
		{
			...
			printf("%d,%d,%d\n\n\n", Sum_green[0], Sum_green[1], Sum_green[2]);
